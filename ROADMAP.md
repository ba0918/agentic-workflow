...
`main` にあります。いまは Phase 5（記録と回復）の途中です。最初の一枚（5.1）は実装と
review を終えて `main` に取り込みました（実行 `20260824t155951-572eaea4`、review の指摘
6 件をすべて閉じて merge）。手順書 `20260825002031` は完了ですが、索引から外す操作は 5.4 で
決めるまで無いので、索引には残っています。

2026-08-25 に 5.2 の brainstorm を行い、「現在地を導く前に、止まった実行を続けられるように
する」ほうが先だと分かったので、5.2 を 2 つに割りました。新しい 5.2 は「止まっても続けられる」
（下の輪郭）、旧 5.2 は 5.3 に、旧 5.3 は 5.4 になりました。5.2 の仕様書の改訂（`workflow.md`、
`implement.md`、`review.md`、`plan.md`）はこの ROADMAP と一緒に承認済みで、次は 5.2 の plan
です（手順書は implement 側と review 側の 2 本）。
...
| 5.2 止まっても続けられる | 手順書や仕様書を途中で直しても、予定に無いコミットや変更があっても、実行をゼロからやり直さずに続けられる | 手順書を改訂して束ね直した実行が、文面の変わっていない手順をやり直さずに完了する。仕様書の改訂で止まった review が、束ね直しで再開する |
| 5.3 現在地の導出と提示 | 新しいセッションが「いまどの切れ目にいて、次に何を決め、どのコマンドで再開するか」を 1 回で知れる | 各中断点で導出結果が実際の位置と一致する。brainstorm の切断からの復元もここで実証する |
| 5.4 記録の寿命と片づけ | 記録が溜まっても、古い・曖昧なものが黙って使われない | 寿命切れの記録を読ませたとき機械が止まる |

順序は 5.1 → 5.2 → 5.3 → 5.4 です。現在地の導出が案内する「再開の先」が死んでいては意味が
無いので、続けられるようにするのが先です。導出は記録が揃わないと書けず、寿命は何を読むか
決まってから決めます。

5.4 には、手順書そのものの寿命も含めます。索引（`open-plans.json`）は完了状態を持たない設計
なので、完了した手順書は索引から外れる必要がありますが、いまは外す操作が無く、完了済みの
手順書が「保留（未完）」として索引に残ります（5.1 の手順書を現在対象にしたとき、完了済みの
`20260823200534` を保留にして切り替えました）。

5.4 の動機は片づけではなく、**AI の検索範囲の衛生**です。終わった手順書や古い記録が作業
...
その原則の下で 5.4 の brainstorm の出発点にする案: 終わった手順書（完了、または取り下げ）は、
...
境界を持つ）、実行の証拠と review の記録の寿命、5.3 が完了を導く規則を「閉じたコミットが履歴に
あるか」にするかは、5.4 の brainstorm で決めます。
...
同じ調べで、次も見つかりました。どちらも 5.2 の brainstorm で決め、5.2 の仕様書の改訂に
入っています（下の「5.2 の輪郭」）。

- **範囲外の未コミット変更の扱い。** `implement.md` の再開の段落は「範囲外の変更はコミット
  されずに残る」と約束しているが、補助スクリプトの `context` は作業ディレクトリに範囲外の
  変更が 1 つでもあると一律に拒否していた。規則を「範囲内の未コミット変更は止める理由、
  範囲外は人に見せる事実」の 1 本にし、スクリプト側を直す
- **ワークフローの側で足りていなかった約束。** 承認済みの仕様書に反するバグは直接直して
  よい（軌道修正の入口 A）/ skill のスクリプトの修理は実行を無効にしない（指紋の対象は
  手順書と仕様書だけ）/ implement が「手順書に無い判断」で止まるのは成果物についての判断に
  限り、道具の不具合や記録の欠けは人に見せる事実。いずれも `workflow.md` の「止まっても
  続けられる」に入れた
...
今回移さない責任:   現在地の導出と提示（5.3）/ 記録の寿命（5.4）/ 会話の handoff（責務外）/
                    修正の往復の進行役（Phase 6）
...
### 5.2 の輪郭（仕様書の改訂は承認済み、plan 待ち）

2026-08-25 の brainstorm で、仕様書とスクリプトの「止まる・拒否する」箇所を全部洗い出し
ました（100 か所あまり。大半は記録の形式エラーと、秘密情報・パスの逸脱・同時書き込みを防ぐ
安全の検査で、これは残します）。問題は、止まると実行ごと失う箇所が 3 つあったことです。

- review は、集合を固定したあとに仕様書の指紋が変わると `findings_stale` を終端の出来事
  として書き、記録の連鎖を二度と伸ばせない。仕様書を戻しても回復せず、implement の実行から
  作り直しになる
- implement は、手順書を改訂すると束ねの指紋不一致で全コマンドが通らず、止まったことすら
  記録できない。「手順書を直したら全部やり直し」の正体
- implement の終端は、証拠で説明できないコミットが 1 つでもあると通らない。再開はそれを
  許すので、「再開はできるが永遠に終われない」実行ができる

どれも「指紋の不一致で止まる」と「二度と続けられない」を同じ物にしていたのが原因です。
指紋の検査そのものは正しく（承認していない内容で機械を動かさないため）、変えるのは
「止まったあとに何ができるか」です。

```text
利用者が得るもの:   手順書や仕様書を途中で直しても、予定に無いコミットや変更があっても、
                    実行をゼロからやり直さずに続けられる。軌道修正の労力は「直したい物を
                    直す」と「続ける」の一言だけ
今回移す責任:       読むことと止まることは指紋に関係なくできる（残っている実行を見せる、
                    記録から組み立て直す、止まったと記録する）/ 指紋の不一致は「新しく
                    始める / 束ね直して続ける」の 2 択で、束ね直しは人が改訂後の手順書・
                    仕様書を承認したうえで rebound の出来事を 1 つ記録し、同じブランチと
                    証拠のまま続ける / 手順の突き合わせは番号ではなく文面の指紋で行い、
                    文面が同じ完了済み手順は持ち越す / 軌道修正の 3 つの入口（A: 承認済み
                    仕様書に反するバグは直接直してコミット、B: 手順書を改訂して束ね直す、
                    C: brainstorm で仕様書を改訂してから B）/ 終端は説明できないコミットと
                    範囲外の変更を列挙して人が 1 回承認する / 未コミット変更の規則 1 本
                    （範囲内は止める理由、範囲外は事実）/ review の findings_stale を終端に
                    せず、束ね直しで stale の指摘を open に戻す / plan の切り替え時の作業
                    ツリーの汚れ検査を落とす
今回移さない責任:   現在地の導出と提示（5.3）/ 記録の寿命（5.4）/ 参照する節の文面で stale
                    の指摘を選り分けること（節単位の指紋が要る。要るまで作らない）
配布の単位:         既存 skill ba0918-implement と ba0918-review の改訂（スクリプトと補足）、
                    ba0918-plan の切り替え検査の削除。新しい skill は無し
動かし方:           implement の既存コマンドに束ね直しの選択肢が増え、終端に人の承認が入る。
                    review の bind に束ね直しが増える
機械的な検証の方法: implement と review のテスト群への追加。仕様書と skill 本文は人が読む
人が判断する項目:   仕様書と手順書の承認 / 束ね直すか新しく始めるか / どの入口か（AI は
                    仕様書に書いてあるか無いかを根拠つきで言う）/ 終端で列挙されたコミットと
                    変更の承認 / 束ね直しで open に戻った指摘の却下・保留
実測の要否と予算:   不要
このフェーズの止まる条件: 束ね直しの出来事の形が既存の証拠の連鎖と両立しないと分かったら、
                    implement 側で吸収せず brainstorm に戻す
```

手順書は 2 本に分けます: implement 側（束ね直し、終端の承認、範囲の規則）を先に、review 側
（`findings_stale` を終端にしない、束ね直しで戻す）を後に。仕様書の改訂は一度に承認して
あります。1 つの brainstorm から 2 本の手順書が出るので、5.3 で作る「plan 待ちの検出」は
「1 本だけ plan 済み」を見分けません。出来事の記録の形式（フィールド名など）は、既存の出来事と
同じ方式で実装が決めます。

観測できる成功: 手順書を改訂して束ね直した実行が、文面の変わっていない手順をやり直さずに
完了する / 証拠の外のコミットがある実行が、人の承認を経て終端に達する / 仕様書の改訂で
`findings_stale` になった review が、束ね直しで再開して完了する / 範囲外の未コミット変更を
持ったまま `context` が通る / 手順書の指紋が変わっていても `stop` が記録できる。

反例: 束ね直しの記録が無いまま、改訂後の手順書で書き進められない / 人が承認していない
指紋では束ね直せない / 範囲内の未コミット変更を残したまま終端に達しない / 束ね直しで `open`
に戻った指摘が、理由の無い却下で閉じない / 文面が変わった完了済みの手順が、やり直さずに
持ち越されない。

### 5.3 の輪郭（5.2 の brainstorm で決めた分。細部は 5.2 のあとに決める）

5.2 の brainstorm は 5.3 の題材から始まったので、5.3 について決まったことがあります。
5.3 の brainstorm はここから再開します。

決めたこと:

- resume（仮の名前 `ba0918-resume`）の責務は「ワークフローを途中で中断していた場合に、
  速やかに途中から再開できる」こと。次の予定を教える skill ではない。読み取り専用で、自分
  では再開せず、各 skill の再開の入口へ送る
- ROADMAP は読まない。幹に途中の物が無ければ「途中の物は無い」で終わり、次に何をするかは
  人が ROADMAP を読んで決める
- 完了した手順書（実行が全手順完了、review 完了、コミット列がメインのブランチから辿れる）
  は、索引が `current` と言っていても「途中」ではない。索引の `current` は候補に過ぎず、
  途中かどうかは実行の記録 → review の記録 → git で導く。索引から外すのは 5.4
- 保留（`held`）の手順書に残った実行は、最後の出来事が `stopped` でも「途中」ではない。
  保留は人が「いまはやらない」と判断した記録だから。`stopped` 単独は中断であって中止では
  ない（5.1 の前の実行は `stopped` → `resumed` を 3 回繰り返して完了している）
- フェーズの間の切れ目も「途中」として検出し、それぞれ再開できるようにする。手順書は
  承認したが implement を始めていない状態は「plan 済み、implement 未開始」として案内する
- brainstorm → plan の切れ目は「brainstorm を plan 化していない」状態として検出する。
  仕様書は状態を持たない（「plan 待ち」の印を仕様書に残さない）。代わりに、brainstorm の
  wrap が成功したとき、進捗ファイルを消す代わりに「セッション ID、反映した正本のパスと指紋、
  plan に進める判定」の記録を `.agents/artifacts/ideas/` に残し、手順書は元になった
  brainstorm のセッション ID を見出しに持つ。plan 待ちは「進める判定つきの wrap 記録の
  うち、どの手順書からも参照されていない物」として導く。plan は brainstorm の記録を消さ
  ない。要る改訂は `brainstorm.md`、`plan.md`、`workflow.md` と対応する補助スクリプト
- 利用者価値の言い方: 復帰でストレスを感じないこと。切れ目は消さず、いまどの切れ目にいる
  かと再開のコマンドを resume が言う

却下した案: ROADMAP の現在地から「次は 5.x」を導く（次の予定は人の判断で責務外）/ 仕様書に
plan 待ちの印を残す（仕様書は状態を持たない）/ wrap の記録と手順書を仕様書の指紋で突き
合わせる（仕様書が改訂されると対応が取れない）。

まだ決めていないこと（5.3 の brainstorm で決める）:

- 提示の形。「いまどの切れ目・次に何を決めるか・どのコマンドで再開するか」を 1 回で見せる
  出力の中身と、skill ごとの再開の入口の対応
- brainstorm の進捗ファイル（セッション ID 単位で、手順書 ID の根の外にある）を resume が
  どう見つけ、切断からの復元をどう実証するか
- 新しい段が幹に増えたとき（たとえば Phase 7 の `doc-check`。旧ワークフローで最後に実装と
  文書の乖離を確かめていた段）、その段が残す記録と再開の入口を 1 組足すだけで resume が
  拡張できる形。doc-check を移すときは「review 完了 → doc-check 待ち」の切れ目が記録から
  導ける形にする
...
`review-testing`。1 つの skill を 1 つの移行単位にします。`doc-check` を移すときは、5.3 の
resume がその段の切れ目を導けるよう、段が残す記録を先に決めます（5.3 の輪郭を参照）。
