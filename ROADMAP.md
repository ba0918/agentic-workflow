...
- 工程を既定で足さない形に全体を替えた（2026-09-19）。実測 1 タスク約 140 万トークン、agent 16 体（指摘ゼロでも最小 6 体）という費用の出所が、どの skill にもコストを下げる分岐が無いことだと分かったための改訂である。既定を反転し（足すのに 1 行の理由が要る）、reviewer は既定 1 体、2 回目のフルレビューは波及しうるときだけ、iterate の判定役と実装の委譲は条件付き、plan と brainstorm の仕上げのレビューも条件付きにした。判断に任せない物は 3 つ（秘密、公開・配布・版、不可逆）だけで、どれも機械の検査か既存の停止規則で受けるので工程は増えない。レビューには必ず走る条件を置かない。抜けたら人が後から頼める。実測はまだ取っていないので、次に 1 周したときに agent の体数とトークンを見る

## 次

- 工程を既定で足さない改訂の効果測定。1 タスクあたりの agent 体数とトークンを、改訂前（16 体 / 約 140 万）と比べる。`evals/` の scenario を回し直して lock を取り直すのも同時に
