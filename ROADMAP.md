...
- reviewは、指摘の確かめ方として実際に走らせた操作をそのまま記録する。記録できる操作を
  固定の一覧に限る検査と、runtimeが持つ秘密情報の正規表現検出は、行為を制限できず正直な
  作業を妨げるため廃止した（`docs/spec/workflow.md`「文字列照合で行為者を縛らない」）。
  runtimeはこの仕様へ追従中で、planとcycleで実装する。
