...
方向として決めたこと: 終わった手順書（完了、または取り下げ）は、人が閉じると判断した時点で、
**手順書の全文をコミットメッセージに残して削除する**。手順書は `.agents/` にあって git に
入っていないので、これは削除というより、手順書が初めて git の履歴に残ることになります。
メッセージは整形せずそのまま（verbatim）入れ、`Plan-ID:` と結果を trailer に書きます。そうすると
削除後も、実行の証拠が持つ手順書の指紋を、git の履歴から取り出した本文で確かめられます。
実行の証拠と review の記録は消しません。閉じる操作をどこに置くか（plan は git を操作しない
境界を持つ）と、5.2 が完了を導く規則を「閉じたコミットが履歴にあるか」にするかは、5.3 の
brainstorm で決めます。5.2 は読み取り専用の小さな専用 skill（仮の名前 `ba0918-resume`）にし、自分では
