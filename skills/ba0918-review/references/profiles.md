...
guidance whose core intent remains recoverable. `info` is optional wording, organization, or
readability improvement with no incorrect behavior.
...
`info` is optional clarity or maintainability improvement with no current behavioral failure.
