...
- 2 本目の期待は次のとおり。baseline の後に commit が無い。応答が `docs/spec/iterate.md#収まらないときの案内` の 3 つの要素を持ち、外れた条件に 4 を含む。仕様書が無いので `/ba0918-brainstorm` を案内し、行き先として `/ba0918-plan` を案内しない（案内の表の説明としての言及は数えない）。「それでも続ける」の選択肢が無い
