...
意味を変えない直しは本文を書き換えます。何をなぜ直したかはコミットメッセージにあります。
**どの版でも、作る物・禁止・人が判断する場面は版 1 のままです。**

| 版 | 直したこと | コミット |
|---|---|---|
| 2 | 手順 2 の変更してよいファイルに `repository.py` と `cli.py` を足した | `7cb0583` |
| 3 | 手順 8 の変更してよいファイルに implement 側の複製を足した | `07d79c1` |
| 4 | 手順 6 の検査コマンドの対象を plan 側に絞った | `40349ed` |
