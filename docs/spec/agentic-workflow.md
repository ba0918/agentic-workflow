...
`WF-033` 正本specを承認する前に、代表scenario、高影響判断、禁止、agentが追加した仮定、有力だが却下した代案、残るrisk、未解決事項、委任事項、後続の自動処理、brainstormを再開すべき境界を人間へ提示しなければならない。これらの判断材料は各項目の要点にとどめ、詳細は草稿本文の該当箇所を指す。判断材料は承認対象ではなく、承認対象は草稿fileのbytesだけである。
...
`WF-039` brainstorm中に製品code、plan、未承認の正本specまたはROADMAPを変更してはならない。そのbrainstorm自身のprogress保存、`.agents/tmp/ideas/`配下への承認前草稿の一時保存、読み取り専用調査だけを許可する。
...
`WF-053` 正本specまたはROADMAPの草稿は、正本へ書き込む内容と同一bytesの一時fileとして`.agents/tmp/ideas/`配下へ保存し、そのpath、content identity、正本の保存先をchatで人間へ提示しなければならない。草稿の全文をchatへ複製してはならず、一時fileに承認前であることを示すヘッダやfrontmatterを加えてはならない。

`WF-054` 人間の承認は、提示したcontent identityに結び付けなければならない。承認時点で一時fileのidentityが提示したidentityと異なる場合（人間による編集を含む）、古い草稿として拒否し、正本化してはならない。修正は対話で受け、新しい草稿を提示する。

`WF-055` 正本化は、一時fileを正本の保存先へ移動し、読み戻したidentityが提示したidentityと一致した後にだけ成功として扱わなければならない。承認成功時は一時fileを残さず、却下・失敗時は再開用に残す。古い一時fileを自動削除してはならない。

`WF-056` 既存の一時fileを置き換える場合、置き換え対象のcontent identityを呼び出し側が明示しなければならない。明示したidentityと既存fileのidentityが一致するときだけ置き換え、一致しない場合は上書きせず停止して人間へ提示する。識別していない一時fileを削除してはならない。一時保存に失敗した場合、草稿提示を成功として扱ってはならない。
