...
- **検証の対応（`Verification coverage`）**: 承認済み仕様書のパスと節、対応する手順番号、
  その手順の完了を示す種類。要求本文は書き写しません
- **予定する変更範囲（`Scope`）**: ファイルの木。範囲外の変更が必要になったときに、その理由と
...
型検査、lint など、周辺を壊していないことを示す補助検証だけでは完了にできません。複数の検証は、
`test` なら portfolio を含む 1 つのプロジェクトのテスト実行コマンド、`check` と `artifact` なら
既存 evidence の checks 一覧、`external` なら既存の短い結果要約と条件を満たしたかの判定へまとめます。
...
記録を読み込む境界では、壊れた入力から誤った作業を再開しないために、読み取れる形式か、必要な
項目があるか、内容の関係と状態の移り方が矛盾していないかを検査します。たとえば、同じ ID の
...
増えたときは、Git の照合をまとめて行う、必要な範囲だけ読む、表示を分割するなど、保存内容を
失わない方法で処理量を抑えます。
