...
**人が止まるのは 2 か所だけです。**brainstorm から plan へ移るときと、plan から cycle へ
移るときです。どちらも「人が読んで承認した物」を次へ渡す場所で、承認が無ければ次は動きません。

cycle の中（implement → review → 修正 → review）は止めません。回し終わったところで、
直した指摘と直さなかった指摘をまとめて人に見せます。往復のたびに人を呼ぶと、人は中身を
見ずに「続けて」と答える役になるからです（下の「承認を儀式にしない」）。
...
止まります。次に同じ手順書で cycle か implement を動かすと、残っている作業を見せて、
「そのブランチで続ける」か「新しいブランチで最初からやる」かを人に聞きます。AI が勝手に
消したり、勝手に続けたりすることはありません。手順書や仕様書を改訂したあとでも、人が
「束ね直して続ける」を選べば、同じブランチと証拠のまま続きます（下の「止まっても続けられる」）。

### 実装中に、仕様に書いていないことが必要になった

implement は、そこで止まります。仕様に無いことを AI が推測で埋めて進めることは
禁止です。不足している判断は brainstorm に戻し、人と決めてから、必要なら仕様書と手順書を
改訂して再開します。戻っても、作業ブランチと証拠は捨てません。
...
- **参照する仕様書**: どの仕様書のどの版に基づくか（パスと指紋）。implement は実行前に、
  その仕様書が承認時から変わっていないことを確かめます。変わっていたら止まります
- **変更してよい範囲**: ファイルの木。implement はこの外へのコミットを拒みます
...
| 作った物で示す（`artifact`） | 人や AI が読んで意味を判断する文章 | 決めた内容のファイルが存在し、形式の検査を通り、人が読んで OK と言ったこと |
...
ないからです。「作った物で示す」は、仕様書や skill の本文のように、人が読んで意味を判断
しないと正しさが決まらない成果物のためにあります。ここでは人の承認が要ります。読む物が
無いのに承認を求めることは、この仕組みでは禁止です（下の「承認を儀式にしない」）。
「外で確かめる」は、人の目が要るものです。
...
review の完了は、固定した集合の全指摘が閉じたことから導きます。詳しくは
[review.md](review.md)。
...
plan も implement も review も、仕様書に書いていないことを自分で決めません。必要になったら
止まり、brainstorm に戻します。

### 承認は、人が読んだファイルそのものに対して行う

AI が草稿を一時ファイルに保存し、パスと指紋（内容の SHA-256）を示す。人が自分の道具で読む。
人が「このファイルを正本にしてよい」と言う。AI が指紋の一致を確かめて正本の場所へ移す。
要約を承認の対象にしません。
...
1. **生きている実行**: `git worktree list`。実装は `implement/<実行 ID>` のブランチと
   専用の作業ディレクトリで動くので、git が一覧を持っています。索引を自作しません
2. **その実行がどこまで進んだか**: 実行ごとの状態の写し（`current-status`）を 1 つ読む
3. **review がどこまで進んだか**: review の記録
4. **取り込まれたか**: git（実装のコミットがメインのブランチから辿れるか）

**未完了の手順書の索引は置きません。**手順書は git で管理し、完了したら作業ツリーから
削除します。作業ツリーに残っている手順書が、そのまま未完了の一覧です。

### 状態の写し（current-status）

実行ごとに 1 つ、`.agents/artifacts/executions/<手順書 ID>/<実行 ID>/current-status` に
...
写しは正本ではありません。出来事の記録が正本で、写しはそこから導いた結果です。食い違ったら
写しを捨てて作り直します。正本が 1 つなので、写しがいくつあっても矛盾しません。
...
**人を止めるのは、明確に道を外れたときだけ**です。道を外れたとは、次の 3 つを言います。

- 成果物についての人の判断が要る（作った物を読んで OK か、指摘を受け入れるか、取り込むか）
- 人が承認していない内容で進もうとしている（指紋の不一致）
- 仕様に無い判断が要る（brainstorm に戻す）

これ以外で AI が転んだとき — 道具の使い方を間違えた、凍結したテストと食い違った、記録を
書き損ねた、予定に無いファイルがあった — は、人を呼びません。AI が自分で回復し、何が
起きて何をしたかを記録に残し、あとで報告します。回復できないときだけ人に返します。
...
| 往復が上限に達したあと、続けるか止めるか | cycle の記録 |
...
作業中の状態と記録は、対象リポジトリの `.agents/` 以下に置きます。手順書だけは例外で、
人が承認した文書なので git で管理します。
...
| `.agents/artifacts/plans/`（git 管理） | 正本の手順書 | 完了したら作業ツリーから削除する。過去は git の履歴で読む |
| `.agents/artifacts/executions/<手順書ID>/<実行ID>/` | implement の証拠と、状態の写し | 残す。上書きしない |
...
未完了の手順書の索引（`open-plans.json`）は置きません。作業ツリーにある手順書が未完了の
一覧で、生きている実行は `git worktree list` が答えます。`status.md` や
`session-history.md` のような状態ファイルも作りません。
