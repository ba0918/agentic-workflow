...
設計判断が未決定なら、その時点で人へ返します。不可逆な操作の直前、人の権限を必要とする場合、
または放置すれば被害を拡大させる事故が起きた場合も同様です。大筋を変えない補完や回復可能な失敗は自走し、終端で
...
brainstorm から入ります。AI と対話して、何を作るか、何を作らないかを決めます。人が判断する
場所も決め、仕様書として残します。そのあと plan で手順書を作り、cycle で実装からレビューまでを
回します。人が途中の文書を承認するのは 2 回です。最後に成果物を確認します。
...
候補が一意でも、実装を始める前に残っている事実を人へ見せます。人は「そのまま続ける」、
「束ね直して続ける」、「既定の再開候補から外して新しく始める」のどれかを決めます。決めた
あとは、同じ cycle 内の委譲切り替えで聞き直しません。AI は古い作業を物理的に消しません。
**束ね直す**とは、動いている実装が「どの手順書のどの内容に従っているか」を、改訂後の内容に
張り替えることです（下の「止まっても続けられる」）。
...
写しを持つと二重管理となり、仕様書の改訂ごとに手順書全体の作り直しが必要です。
実際に、写しを求めていた時期の手順書は 788 行になり、そのうち大半が仕様の写しでした。

手順書の中で、implement が機械的に読む入口となる top-level block は次の 2 つです。

- 検証の対応（`Verification coverage`）には、承認済み仕様書のパスと節、対応する手順番号、
  その手順の完了を示す種類。要求本文は書き写しません
- 予定する変更範囲（`Scope`）には、ファイルの木を書きます。範囲外の変更時に、その理由と
  影響を判断する起点です。絶対的な許可リストではありません

手順は `Step 1` から連続する番号を持ちます。検証の対応に載せたすべての仕様節は 1 つ以上の
手順へ結びつき、すべての手順は 1 つ以上の仕様節に基づきます。runtime はこの対応から手順と
完了種別を導くため、起動する側が別に手入力しません。
...
| 作った物で示す（`artifact`） | 人や AI が読んで意味を判断する文章 | ファイルと形式検査を記録し、cycle の完了までに独立レビューを通ったこと |
| 外で確かめる（`external`） | 実機での動作確認、人が頼んだ実測 | 実行結果が記録され、手順書の完了条件を満たしたこと。不可逆な操作や人の権限が要る操作は実行前に人へ返す |

各手順は、要求の反例を検出して仕様適合を直接示す検証を少なくとも 1 つ持ちます。全体テスト、
型検査、lint などの補助検証だけでは完了にできません。これらは周辺を壊していないことを示します。
複数の検証は、`test` なら portfolio を含む 1 つのプロジェクトのテスト実行コマンドへまとめます。
`check` と `artifact` なら既存 evidence の checks 一覧へまとめます。`external` なら既存の短い
結果要約と、条件を満たしたかの判定へまとめます。
subcommand ごとの新しい出来事は作りません。別の実行条件や完了判定を持ち、まとめられない場合だけ
手順を分けます。

runtime が保証するのは、宣言した仕様節と手順の対応、必要な evidence の存在、実行の成功です。
関連する仕様の選定漏れ、節の中の要求漏れ、検証が反例を本当に検出するか、検証方式が適切かは、
plan と review が意味を読んで判断します。要求 marker、永続 ID、形式要求言語は作りません。

文章や skill のように意味を機械判定できない成果物は `artifact` を選び、静的検査、独立レビュー、
終端確認を使います。文章へ LLM を実際に走らせる確認は、人が scenario、model、token 予算を
明示した場合だけ `external` として行い、実施していない場合は検証済みと表現しません。
...
- 作業ブランチは専用とし、実装には専用の作業ディレクトリ（git worktree）も使います。
  メインのブランチは触りません。
- コミット列は、ステップごとにコミットされた変更です。
- 証拠には、どのステップでどのテストが失敗し、どう通ったかを記録します。コミットした物も記録します。
  1 つの出来事につき 1 ファイルで、追記だけで上書きしません。

review は差分に加え、証拠から機械的な完了を確かめ、plan に宣言した検証が実行されて仕様適合の
直接証拠になっているかを辿ります。証拠に成功とあるだけでは、仕様適合を結論にしません。実行 ID
の無い単独 review はブランチか差分だけでも動きます（[review.md](review.md) の「受け取るもの」）。

### review → 修正 → review: 指摘の集合

review が渡すのは、確かめ方から機械的に導いた ID を持つ指摘の集合です。最初に実装全体を見て、修正のあとは
...
手順書の Scope は予定との差を判断する起点であり、絶対的な許可リストではありません。仕様の意味を
変えず、既存の依存、環境、承認範囲で行える安全な通常補完は、Scope 外でも理由と影響を記録して
修正を続けます。新しい依存、重要な設計判断、外部環境、権限、危険な対象が必要な指摘だけを、人が
決める指摘（`human_judgment`）にします。指摘のテキストだけを根拠に範囲を広げません。
...
plan、implement、review はいずれも、仕様書に書いていない重要な設計判断を自分で決めません。
...
仕様書が変わると、それを参照する手順書と実行中の証拠の根拠が古い版になります。
...
### AI を実際に走らせる確認（実測）は、条件が明示されたときだけ

文章に対して LLM を走らせて振る舞いを確かめることは、既定では要求しません。人が scenario、
model、token 予算の 3 つを明示した場合だけ行います。LLM 以外の外部実測は、承認済み仕様と
手順書の条件に従います。

### 機械の検査は、現実との境界と壊れた入力を見る

補助スクリプトが行う検査は、**外の世界との照合**と、**読み込む記録が安全に解釈できるかの
確認**に限ります。外の世界との照合には、次のようなものがあります。

- 人が承認した内容と、いま手元にある内容が違う
- 手順書の範囲外のファイルに書こうとしている
- パスが安全でない
- 人の判断が無い
- 人の読んだ物が変わった、または人が却下した
- テストの失敗が「何かエラーが出た」で終わっていて、承認された未実装を指していない

記録を読み込む境界では、壊れた入力から誤った作業を再開しないために、読み取れる形式かを
検査します。必要な項目の有無や、内容の関係と状態遷移の矛盾も検査します。たとえば、同じ ID の
重複、存在しない項目への改訂、許されていない順序の出来事は拒否します。検査規則は記録の種類
ごとに 1 か所へ置き、同じ判定を複数の補助スクリプトへ重ねません。

一方で、記録そのものを信用するためだけの独自の指紋、出来事を結ぶ指紋の鎖、書いた直後に同じ
補助スクリプトが自分の出力を読み直すだけの検査は作りません。記録は人と AI が読める素朴な形に
し、外の現実を見ない自己完全性の仕組みを増やさないためです。

この記録は、1 人の作業者が自分の実行を続けるための Git 除外済みローカルデータです。作業者が
利益のない手編集や偽造をすることは脅威として扱いません。署名、真正性の証明、改変検出、記録の
同一性確認は作りません。形式や必須項目の検査は、プロセス中断による途中書き込みや壊れた入力で
誤動作しないための通常の正確性であって、記録を書いた本人を疑うための仕組みではありません。

出来事、実装の証拠、review の指摘には、件数や経過時間による保存上限を設けません。これらは
過去の経緯を復元するための記録なので、上限で古い物を落とすと役割を果たせなくなります。件数が
増えたときも保存内容は失いません。Git の照合をまとめて行う、必要な範囲だけ読む、表示を
分割する、といった方法で処理量を抑えます。

### 文字列照合で行為者を縛らない

補助スクリプトは、コマンドや変更内容の文字列を照合して、AI や人の行為を制限しません。
以前は、review が記録できる操作を固定の一覧に限り、コミットや記録に秘密らしい文字列が
含まれていれば拒否していました。どちらもやめました。ファイル名（パス）を見る危険な対象の
検査は、これに含みません。中身ではなく、書き込む先を見る検査だからです。

理由は、文字列の照合では行為を制限できないからです。補助スクリプトは何が実行されたかを
観測できず、記録の文字列を縛っても実行は縛れません。秘密の検出も、正規表現では主要な
形式の多くを見逃す一方、テストや仕様書の例を誤検知して正直な作業を止めます。そのうえ
「検査を通した」という記録が残るため、通っていない物まで通ったように見えます。縛れない
検査は、無いより悪い結果を招きます。

実行を制限する責務は、実行環境（sandbox、permission）にあります。秘密を混入させない責務は、
コミットや記録を書く行為者（AI または人）にあります。その規則は、このワークフローの外にある、
利用者が自分の AI へ与えている秘密情報の扱いの規則に委ねます。本格的な秘密の検出が要る
プロジェクトは、専用の検出器を手順書の検査コマンド（[plan.md](plan.md) の「完了の示し方」）に
書きます。この仕組みは検出器を内蔵しません。
...
1. 生きている実行は `git worktree list` で調べます。実装は `implement/<実行 ID>` のブランチと
   専用の作業ディレクトリで動くので、git が一覧を持っています。同じ一覧を自分で作りません
2. その実行がどこまで進んだかは、実行ごとの状態の写し（`current-status`）を 1 つ読んで調べます
3. review がどこまで進んだかは、review の記録で調べます
4. 取り込まれたかは、実装のコミットをメインのブランチから辿れるか、git で調べます

**未完了の手順書の一覧は、どこにも書きません。** 手順書は git で管理し、完了したら作業
ツリーから削除します。作業ツリーに残っている手順書が、そのまま未完了の一覧です。

新しい cycle を始める前には、同じ手順書の未完了な実行が残っていないかを確認します。残って
いたら実装を始めず、手順書、実行 ID、開始時刻、最後の出来事、完了済みと残りの手順、ブランチ、
作業ディレクトリ、未コミット変更を人に見せます。この確認は実装中の停止ではなく、開始前に
「前の作業は何だったか」を思い出し、続けるか片付けるかを決める自然な対話です。

片付けは 2 段階に分けます。通常の片付けは、その実行を既定の再開候補から外すだけで、出来事、
証拠、ブランチ、作業ディレクトリを残します。実行 ID を明示すれば後から調査・復帰できます。
証拠、ブランチ、作業ディレクトリを物理的に消す操作は別にし、対象を明示して人が承認した場合
だけ行います。経過時間や AI の推測だけで、放棄された実行だと決めません。
...
- 機械的に導ける 4 つだけを書きます。手順書のパスと版 / 完了した手順 / 最後の出来事および
  その理由 / ブランチと作業ディレクトリ
...
これ以外で AI が転んでも、人を呼びません。たとえば、道具の使い方を間違えた場合や、凍結した
テストと食い違った場合です。記録を書き損ねた場合や、予定に無い通常のファイルが必要になった
場合も同様です。
AI が自分で回復し、必要な事実を記録して、あとで報告します。人の判断なしには回復できないと判明したときだけ
人に返します。
...
- 文書が変わっていても、読むことと止まることはできます。残っている実行を見せる、記録から
  状態を組み立て直す、止まったと記録する、の 3 つが該当します。手順書や仕様書が承認時から変わって
  いても通ります。書き進める前にだけ、承認コミットと現在の内容の意味を比較します
- 文書が書き換わっても、やり直しにはなりません。答えは「新しく始める」と「束ね直して
...
- 予定に無い通常の変更は、止める理由ではなく、人に見せる事実です。予定範囲の外にある通常の
  ファイル、補助スクリプトの不具合、記録の欠けは、機械が一律に弾きません。大筋を変えずに
  補えるなら理由を記録して続け、終端で報告します。本番設定、データ移行、アクセス制御、
  秘密情報など、誤ると被害が大きい対象は編集前に人へ返します
- 危険なパス、一時ファイル、ログ、生成物の検査は、予定範囲の外でも迂回せず、すべての
  コミットで同じように通します
- implement が「手順書に無い判断」で止まるのは、重要な設計判断に限ります。道具の不具合、
  記録の欠け、合意済みの結果を実現する内部的な選択は、brainstorm に戻す理由ではありません

### 文書が書き換わったとき

手順書と仕様書は、実行の途中で書き換わります。書き損じが見つかればその場で直しますし、
仕様書が改訂されればそれに基づく手順書も追いかけます。

**書き換わったという事実そのものは、人へ返す理由になりません。** Git の差分があるという観測は、
...
| 新しい実行の開始前に、残っている実装を続けるか、束ね直すか、片付けるか | implement の出来事（続き、束ね直し、または再開候補から外した記録）。実装を始めたあとは、道を外れた止まり方のときだけ人に返す |
| 軌道修正をどの入口（A / B / C）で行うか | 結果が記録に現れる: A は作業ブランチの修正コミットと cycle の終端報告、B は手順書の新しい版と束ね直し、C は仕様書の改訂 |
| 証拠で説明できないコミットと範囲外の変更を、意図した物と認めること | cycle の終端報告で確認し、取り込む判断は git（merge コミット）に残る |
...
置かないものが 3 つあります。1 つ目は未完了の手順書の一覧です。作業ツリーにある手順書が
その役割を持ちます。2 つ目は生きている実行の一覧です。これは `git worktree list` が答えます。
3 つ目は `status.md` や `session-history.md` のように、進み具合を書き写した状態ファイルです。
どれも別の場所にある事実の写しなので、元とずれます。
