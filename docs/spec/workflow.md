...
「束ね直して続ける」を選べば、同じブランチと証拠のまま続きます。**束ね直す**とは、動いて
...
- **参照する仕様書**: どの仕様書のどの版に基づくか（パスと指紋）。ここでいう指紋とは、
  ファイルの中身から計算した SHA-256 のハッシュ値のことで、1 バイトでも違えば別の値に
  なります。implement は実行前に、その仕様書が承認時から変わっていないことを、この値を
  比べて確かめます。変わっていたら止まります
...
AI が、置き場所の決まっているファイルをそこに直接書き、パスと指紋と、変更前と比べる差分を
示す。人が自分の道具で読む。人が「このファイルでよい」と言う。AI が指紋の一致を
確かめてコミットする。要約を承認の対象にしません。

承認前の内容は、作業ツリーにあってコミットされていない状態です。承認とコミットが同じ
...
- 人が承認した内容から、**設計の判断が動いた**（下の「文書が書き換わったとき」）
...
- **予定に無い事実は、止める理由ではなく、人に見せる事実。** 範囲外の未コミット変更、証拠で
  ...
  終端で列挙し、人が承認します。skill のスクリプトは指紋を比べる対象ではないので、実行中に
  修理しても実行は無効になりません
...
**書き換わったという事実そのものは、人に返す理由になりません。**指紋が違うという観測は、
「何かが変わった」としか言っていません。それを止まる根拠にすると、誤字の修正と設計の
差し替えが同じ扱いになり、前者のたびに人が呼ばれます。

代わりに、AI が中身を読んで判定します。**設計の判断が動いたか**です。設計の判断とは、
作る物、禁止していること、人が判断する場面、参照する仕様書、変更してよい範囲の 5 つを
指します。
...
| brainstorm | 仕様書を書いて承認待ち | コミットされていない仕様書と記録、作業ファイル | 差分と指紋を出し直す |
| plan | 手順書を書く前 | 何も残らない | 最初からやり直す（失う物は無い） |
| plan | 手順書を書いて承認待ち | コミットされていない手順書 | 差分と指紋を出し直す |
...
| 仕様書・手順書・ROADMAP の承認 | ファイルそのもの（承認した指紋の内容がコミットされる） |
