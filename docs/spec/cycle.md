...
その間は原則として人を呼びません。重要な設計判断が未決定なら人へ返します。不可逆な操作の直前、
人の権限を必要とする場合、または放置すれば被害を拡大させる事故が起きた場合も同様です。原因診断と
安全な別手段を 1 回試しても進捗しない場合も人へ返します。詳しい境界は下の「人に返す場合」にあります。
...
実装を委譲する前にその事実を人へ見せます。続ける、束ね直す、再開候補から外して新しく始める、
のどれにするかを対話で決めます。この開始前確認は、実装中の往復を止める人間ゲートには数えません。
一度方針を決めたあとは、同じ cycle 内の委譲切り替えで聞き直しません。証拠、ブランチ、作業
ディレクトリの物理削除は cycle の責務外で、別の明示的な掃除操作として扱います。

## 実装を委譲する

cycle は委譲前に、実行 ID、承認コミットから作るブランチと作業ディレクトリ、実行ごとの記録
場所を作り、対象の手順書と開始時の Git コミットを結び付けて記録します。そのあと、実装を別の会話に委譲します。**cycle を動かしている会話の文脈を、
...
ロックと調停は要りません。同期であることが排他になっています。
...
reviewが、形だけのtest、coverageの意味上の漏れ、実行されていない検証をfindingにする場合が
あります。仕様の意味を変えず、既存の依存、環境、承認範囲で補強できるなら、修正を委譲して
往復を続けます。
planの検証方式が不足していても、承認済み設計の中で選択が一意なら、逸脱理由とevidenceを残して
止まりません。

要求が曖昧、完了の判定基準が無い、新しい依存、外部環境、権限、危険な対象が必要な場合だけ、
何が仕様から決められないかを示して人へ返します。
...
- 危険なパス、一時ファイル、ログ、生成物の検査は、予定範囲の内外にかかわらず
  すべての修正コミットで通します
...
- cycle が実行 ID、ブランチ、作業ディレクトリ、記録場所を作り、委譲開始と委譲終了を実行の
  出来事として記録します
