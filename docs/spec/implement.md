...
実行の前に、その手順書が参照している仕様書の指紋（内容から計算した SHA-256 のハッシュ値。
1 バイトでも違えば別の値になります）が、手順書に書かれたものと一致することを
確かめます。
...
- 手順書か仕様書が書き換わっていて、**設計の判断が動いた**（下の「文書が書き換わったとき」）
- 作った物を人が拒否した
...
**違っているという事実そのものは、人に返す理由になりません。**指紋が違うという観測は、
「何かが変わった」としか言っていません。それを止まる根拠にすると、誤字の修正と設計の
差し替えが同じ扱いになり、前者のたびに人が呼ばれます。

代わりに、AI が両方を読んで判定します。**設計の判断が動いたか**です。設計の判断とは、
次の 5 つを指します。
...
決めます。どれも動いていなければ、何がどう変わったかを `recovering` として記録し、
そのまま続けます。終端で、証拠で説明できないコミットと同じ箱に入れて人に見せます。
