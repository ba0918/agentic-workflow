...
3. `check` の出来事を記録する（走らせたコマンドと終了コード、手順書の範囲内で変わった
   ...
   いる変更**です。検査コマンドが変えた物だけではありません。この一覧が空でないのに
   コミットが無ければ、その手順は未完了です（下の「完了と引き渡し」）
4. 変えた物があればコミットする

検査の役目は確かめることであって、成果物を残すことではありません。検査だけで完了が決まる
手順には、何も変えないもの（リポジトリ全体の検査が通ることを確かめるだけの手順）があります。
コミットを一律に求めると、そういう手順は完了できなくなります。変えた物があるのに
...
`refactor` → `commit`、`check` は `check` →（変えた物があれば）`commit`、`artifact` は
`artifact` → `approval` → `commit`、`external` は `external` → `approval` →（記録する物があれば）`commit`）、手順書が求める広い検証
