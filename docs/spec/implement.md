...
- **参照する仕様書**: パス。承認コミットのツリーからそのときの版を読みます
- **予定する変更範囲**: ファイルの木。実際の変更との差を見つけ、予定外の変更の理由と影響を
  判断する起点にします
...
張り替えることです。ブランチも作業ディレクトリも証拠も捨てずに、従う対象だけを差し替えます。
新旧の版は Git のコミットとツリーから読み、独自の指紋は持ちません。

同じ実行の中での委譲の切り替わりは、ここには入りません。cycle が起こした次の委譲も、
前のセッションから残っている一意な実行も、重要な設計判断や危険な対象が動いていなければ、
人に聞かずに続きから始めます（[cycle.md](cycle.md) の「実装の委譲は、進捗で切る」）。

### 何を起点に見つけるか

起点は証拠ディレクトリです。`.agents/evidence/<手順書 ID>/` の下にある実行の
...
- 終わっていない実行が一意で、重要な設計判断や危険な対象が動いていなければ、そのブランチで
  自動的に続けます。まず `resumed` の出来事を 1 つ記録し、そのときのブランチの先端、記録で
  説明できないコミットの SHA、未コミットの変更の有無を書きます
- 複数の実行候補を一意に選べない、または重要な設計判断が動いていた場合だけ、人が続け方を
  選びます。「新しく始める」なら、新しい実行 ID でブランチと作業ディレクトリを作ります。
...
- そのブランチで続ける場合、`resumed` の出来事には、ブランチの先端、記録で説明できない
  ...
  問わず安全検査を行います。大筋を変えない通常の補完は理由を記録してコミットし、危険な対象や
  重要な設計判断に関わる変更はステージせず人へ返します
- 「束ね直して続ける」を選んだら、まず改訂前と改訂後の手順書の手順を、番号ではなく見出しと
  本文の内容で突き合わせて、次の表を人に見せます: 内容が同じで完了済みの
...
既存のスクリプトを、それも無ければ標準の道具を調べ、対象のテストを確かめられるものを AI が
選びます。有効なコマンドを決められない場合は、完了条件が欠けた手順書として製品コードを書く
前に止まり、plan へ返します。
...
4. 同じ文脈の自己申告ではなく、独立した review で意味を確かめられる状態にする
5. コミットする。成果物を人が受け入れる判断は cycle の終端へ集約する

### 外で確かめる

実機で動かす、人が頼んだ「AI を実際に走らせる確認（実測）」のように、人の目が要るものです。

1. 手順書に書かれた確認を行う。implement が安全に実行できない、人の権限が必要、または
...
- ステージングはすべて補助スクリプト（`stage`）を通します。秘密情報、危険なパス、一時ファイル、
  ...
  `git add -A` は使いません
- git の hook は無効化しません。hook が失敗した、または hook がファイルを書き換えたなら、
  原因と差分を調べ、安全な通常の修正なら直して再試行します。重要な設計判断や危険な対象に
...
聞きません。前のセッションから残っている実行も、一意に選べて重要な判断が動いていなければ
同じ扱いです。
...
実行の途中で、手順書や仕様書が書き換わることがあります。書き損じが見つかってその場で
直った場合も、仕様書が改訂されて手順書が追いかけた場合も、実行が束ねた内容と手元の内容は
違うものになります。

**違っているという事実そのものは、人に返す理由になりません。**Git の差分があるという観測は、
...
独立レビュー可能な状態 → コミット、`external` は記録 → 条件の判定 →（残す物があれば）
コミット）、手順書が求める
...
| 残っている作業が複数ある、または重要な設計判断が動いた | 束ね直して続けるか、新しく始めるか |
