...
委譲された場合は、cycle が実行 ID、ブランチ、作業ディレクトリ、記録場所を先に作り、結び付けて
から渡します。記録場所の結び付けは、ブランチと作業ディレクトリが既にあることを前提にするため
です。人が implement を直接動かした場合だけ、implement 自身が同じ準備をします。直接動かして
作った実行は委譲用に結び付けられていないので、後から cycle の委譲対象にはなりません。
...
（途中で止まった、会話が切れた、など）。それを見つけたら、残っている内容を見せて、人に
選んでもらいます。この節の発見、提示、`resumed` と `resume-candidate-retired` を記録するのは、
人が implement を直接動かしたときは implement です。cycle から委譲されるときは、委譲前に
cycle が行います。委譲された implement は、cycle が結び付けた実行の到達点から続きを行い、実行を
作らず `resumed` も記録しません。

```mermaid
flowchart TD
    S[implement を起動、または<br>cycle が委譲を準備] --> Q{この手順書の証拠に<br>終わっていない実行がある?}
    ...
    H -- 束ね直して続ける --> RB[implement が rebound を記録し<br>持ち越せない手順から再開する]
...
- ブランチ `implement/<実行 ID>` があるか。あれば、始めたコミットから先端までのうち、
  `commit` の出来事で説明できないコミットの一覧（SHA と件名）。件名はそのまま見せます。
  秘密らしい文字列を探して伏せ字にすることはしません
...
- 「束ね直して続ける」を選んだら、まず改訂前と改訂後の手順を突き合わせます。番号ではなく、
  ...
  `rebound` には改訂後の手順書の予定する変更範囲も記録し、以後の予定範囲の内外の判定はその
  範囲で行います。cycle から委譲されている間に `rebound` を書けるのは委譲された implement
  だけなので、cycle は `delegated` のあとに束ね直しを implement へ指示します。止まっている
  実行には `delegated` を書けないので、cycle は先に `resumed` を記録します。
...
1. 失敗するテストを先に書きます（RED）。その手順で作る振る舞い 1 つについて、小さなテストを
   ...
   失敗する」ことを確かめます。失敗の理由は AI が分類します。
   テストの書き間違い、読み込みの失敗、依存の不足、権限、ネットワーク、無関係な既存の失敗は
   「期待した失敗」ではありません。失敗の内容が `failed` や `exit code 1` のような、何が
   足りないのかを言っていない物であるときも、期待した失敗として受け付けません。失敗の理由が
   「期待した失敗」かどうかを補助スクリプトは判定しません。テストを走らせるのは AI で、
   `stage --phase red` は終了コードを AI から受け取ります。機械的に確かめるのは、受け取った
   終了コードが失敗を示すこと、テストの内容の凍結、そして下の「コミット」と同じステージ
   領域の安全検査です
...
ありません。同じ手順の中で新しい RED を書き直せば（`stage --phase red` をもう一度）、その RED が
新しい凍結になり、続きから進めます。
...
- コミットの作法は、プロジェクトの規則（AGENTS.md など）があればそれに、無ければ
  利用者の共通規則に従います
- ステージングは実装者が `git add <パス>` で個別に行います。`git add .` や `git add -A` は
  使いません。補助スクリプトは、手順の記録（`stage`）とコミットの記録（`record-commit`）の
  ときに、ステージ領域とコミットの内容を Git から読んで検査します。危険なパス、一時ファイル、
  ログ、ビルド成果物は、手順書の予定範囲の内外にかかわらず拒否します。拒否されたパスは
  ステージ領域から外してから、記録をやり直します
- 秘密情報の混入は、補助スクリプトでは検出しません。混入させない責任は実装者にあり、規則は
  このワークフローの外にある、利用者が自分の AI へ与えている秘密情報の扱いの規則に従います。
  専用の検出器が要るプロジェクトは、手順書の検査コマンドに書きます（[workflow.md](workflow.md) の「文字列照合で行為者を縛らない」）
...
- 手順書の予定範囲外の直し（承認済みの仕様書に反するバグを見つけた、など）も同じ検査を
...
| `delegated` / `returned` | 委譲を始めた（実行先とモデル） / 委譲された会話が戻った（結果の要約） |
| `resumed` | 残っていた実行を続けた |
| `resume-candidate-retired` | 残っていた実行を既定の再開候補から外した（理由） |
...
からです。例外が 1 つあります。手順書の予定する変更範囲（`## Scope`）が変わったときは、意味の
判定に関わらず束ね直し（`rebound`）で受けます。範囲は機械が予定範囲の内外の判定に使う物で、
追従の記録（`recovering`）は範囲を持たないため、追従すると古い範囲で判定が続いてしまうから
です。補助スクリプトは、追従先のコミットにある手順書の範囲が承認時と違えば追従を拒否します。同じ理由で、続けた場合も必ず記録に残します。判定を間違えていたとき、人が終端で
気づけるようにするためです。
...
└── scripts/
    ...
    ├── plan_artifact.py            手順書の読み取り（plan と共有する正本の複製）
    ├── implementation_evidence.py  出来事の列から現在の状態を導く共有処理（review と共有）
    ├── git_status.py               git status の解釈（review と共有）
    ├── path_safety.py              危険なパスの判定（review と共有）
    └── runtime/                    責務ごとに 1 モジュール
        ├── cli.py            コマンドラインの受け口
        ├── planning.py       手順書から、参照する仕様書と変更してよい範囲を読む
        ├── repository.py     リポジトリの発見と、ブランチと作業場所の束ね
        ├── documents.py      承認コミットにある手順書と仕様書の照合、文書追従の判断
        ├── evidence.py       出来事の追記、状態の写しの更新、段階・コミット・束ね直しの記録
        ├── events.py         出来事の書き手と順序の検査
        ├── completion.py     全手順完了の導出と、コミット履歴との照合
        ├── safety.py         パスの安全検査
        ├── staging.py        予定範囲との差の判定
        ├── tdd.py            テストの凍結（RED の内容と GREEN・REFACTOR の照合）
        ├── deliverables.py   成果物の出来事の形
        ├── gates.py          宣言された危険な操作の種類
        ├── resume.py         未完の実行の提示と再開、再開候補からの除外
        ├── context.py        上記を skill から呼ぶための公開面（再 export）
        ├── deps.py           共有モジュール（手順書の読み取り、共有導出処理、git status の解釈）の読み込み
