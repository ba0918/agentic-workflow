...
同じか」を確かめます。どれかが変わっていたら、その手順を進めずに止まります。作業ディレクトリ
...
次は止まる理由ではありません。事実として記録・提示し、人が判断します: 範囲外の未コミット
変更、証拠で説明できないコミット、補助スクリプトの不具合（指紋の対象は手順書と仕様書だけ
なので、実行中にスクリプトを直しても実行は無効になりません）、記録の欠け（後から記録します）。
「手順書に無い判断」で止まるのは、成果物についての判断に限ります。
