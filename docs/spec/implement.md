...
- ステージングは実装者が `git add <パス>` で個別に行います。`git add .` や `git add -A` は
  ...
  ときに、ステージ領域とコミットの内容を Git から読んで検査します。秘密情報、危険なパス、
  一時ファイル、ログ、ビルド成果物は、手順書の予定範囲の内外にかかわらず拒否します。拒否された
  パスはステージ領域から外してから、記録をやり直します
...
└── scripts/
    ...
    └── runtime/                    責務ごとに 1 モジュール
        ...
        ├── safety.py         パスと変更内容の安全検査
        ├── staging.py        予定範囲との差の判定
        ├── secret_detect.py  秘密らしい内容の検出
