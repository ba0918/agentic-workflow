...
`PL-030` plan skillは、正本へ書き込む内容と同一bytesの草稿を`.agents/tmp/plans/`配下の
一時fileとして保存し、そのpath、content identity、正本の保存先、確認してほしい判断事項を
chatで人間へ提示しなければならない。草稿の全文をchatへ複製してはならず、要約だけを
確認対象にしてもならない。
...
`PL-033` 一時草稿fileには、承認前であることを示すヘッダやfrontmatterを加えてはならない。
一時fileを`open-plans.json`へ登録してはならず、後続工程はそれをcurrent planとして
解決してはならない。

`PL-034` 人間の承認は、提示したcontent identityに結び付けなければならない。承認時点で
一時fileのidentityが提示したidentityと異なる場合（人間による編集を含む）、古い草稿として
拒否し、正本化してはならない。修正は対話で受け、新しい草稿を提示する。

`PL-035` 正本化は、一時fileを正本の保存先へ移動し、読み戻したidentityが提示した
identityと一致した後にだけ索引へ登録しなければならない。不一致は失敗として扱う。
承認成功時は一時fileを残さず、却下・失敗時は再開用に残す。古い一時fileを自動削除
してはならない。

`PL-036` 既存の一時fileを置き換える場合、置き換え対象のcontent identityを呼び出し側が
明示しなければならない。明示したidentityと既存fileのidentityが一致するときだけ置き換え、
一致しない場合は上書きせず停止して人間へ提示する。識別していない一時fileを削除しては
ならない。一時保存に失敗した場合、草稿提示を成功として扱ってはならない。
