...
合流もせず、後から来た内容を別ファイル（`<セッション ID>.conflict-<内容の指紋の先頭>.md`）に
...
壊れた参照、書かれた指紋と中身の食い違い）、推測で補わずに止まって、どこが壊れているかを
示します。
...
書く前に、AI は記録全体を見直します。次のどれかがあれば、書かずに対話に戻ります。
...
- **仕様書（または ROADMAP）** — `docs/spec/` に置きます
- **決まったことの記録** — `docs/agreements/<日付>_<名前>.md` に置きます。中身は上の
...
AI がチャットに出すのは、ファイルのパス、内容の指紋（SHA-256 のハッシュ値）、変更前と
比べる差分を見るコマンド、そして人が判断すべき点の要約だけです。全文をチャットに貼る
ことはしません。人は自分の道具でファイルを読みます。
...
承認は、人が「このパスのこのファイルを、この指紋のまま承認する」と明示したときだけ
成立します。黙っていること、「もう少し話そう」と言うことは承認ではありません。直したい
ところがあれば対話で受けて書き直します。人にファイルを直接編集してもらうことはしません
（編集すると指紋が変わり、人が読んだ内容と承認する内容が別物になります）。

承認されたら、AI はファイルの指紋が提示したものと同じであることを確かめてから、
そのファイルだけをコミットします。**承認とコミットは同じ操作です。**コミットが承認の
記録になるので、承認を別の場所に書き留めることはしません。

コミットまで終わったら、対話中の作業ファイル（`.agents/tmp/ideas/`）を消します。決まった
ことは `docs/agreements/` に残っているので、作業ファイルを残す理由はありません。

指紋の確認かコミットで失敗したら、書いたファイルも作業ファイルも残し、どの段階で失敗した
かを報告します。
...
| 仕様書と記録の承認 | そのファイルを、その指紋のまま承認してよいか |
...
- 指紋の確認かコミットで失敗したら、書いたファイルも作業ファイルも残し、どの段階で
