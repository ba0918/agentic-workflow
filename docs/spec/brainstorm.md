...
「仕様が黙っている」は「実装側に任せる」を意味しません。保存する状態、データの寿命と移行、
外部サービス、必要な権限、新しい外部依存、失敗時の回復など、要件から必要になる判断について、
合意、明示的な委任、または該当しないという結論のどれかが必要です。
...
既存の構造と制約を読みます。必要に応じて、関連するコードや文書を読み取りだけで調べます。
次を含めて、目的そのものを疑えます。

- そもそも作る必要があるか。既存の仕組みや、作らない方法で目的を達成できないか
- 要件から暗黙に必要になる重要な設計判断が、未決定のまま残っていないか
...
- **仕様書** — `docs/spec/` に置きます。戦略 brainstorm の ROADMAP はリポジトリ直下の
  `ROADMAP.md` に置きます
...
ステージまたはコミットで失敗したら、書いたファイルも作業ファイルも残し、どの段階で失敗した
かを報告します。

## 検証できる仕様にする

実装が仕様を満たしたか後から辿れるように、実装が満たす要求は見出しで参照できる粒度にします。
各要求には、観測できる成功条件と、その要求が破られたと分かる反例を持たせます。plan が要求を
検証へ結びつけられないほど節が広い、曖昧、または判定基準が無い場合は、plan に説明を補わせず、
brainstorm で仕様を分けるか明確にします。
...
- 実装が満たす要求を見出しで辿れ、成功条件、反例、または人が判断する基準がある
...
- ステージまたはコミットで失敗したら、書いたファイルも作業ファイルも残し、どの段階で
