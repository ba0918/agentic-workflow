...
**Plan revision:** `2`  
**作成日時:** 2026-08-23 13:37:36 JST（revision 2: 2026-08-23 14:05 JST）  
**置換対象:** revision 1 `.agents/artifacts/plans/20260823133736_review-skill-migration.md`（`sha256:d1dc956f7c133d7075271bef93e5437c9e5802ca6acc027782d762bcd4697de8`）  
**公開先:** `.agents/artifacts/plans/20260823133736_review-skill-migration-r2.md`

**対象仕様:**

- `docs/spec/review-skill-migration.md`
...
三仕様の役割: `review-skill-migration.md`はPhase 4固有の承認済み仕様、`agentic-workflow.md`は
workflow全体の親仕様、`cycle-skill-migration.md`はreviewが受け取る引き渡し物の形を定めるPhase 3の仕様。
...
revision 2は、revision 1の意味を変えずに、cycle runtimeが機械的に読む書式（対象仕様の行と変更範囲の
tree）へ整形したものである。
...
各fileの役割: `review.md`は初回full reviewと差分再reviewの手順、`evidence.md`はfinding eventの形と
保存場所、`profile/default.md`はコード用7観点、`profile/skill.md`はskill文書用、`review_model.py`は
純粋なmodel（finding、ID導出、状態遷移、event schema）、`review_runtime.py`はGit・filesystem adapter
（identity検証、trailer、claim、event書込み、oracle実行）。
