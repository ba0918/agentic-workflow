...
from review_support.validation import bounded_text, review_execution, safe_finding_strings
...
def validate_finding_for_binding(
    ...
    *,
    ...
    active_commit = spec_commit or str(binding.get("spec_commit") or binding.get("approval_commit") or "")
    ...
    mismatch = (
        ...
        or not allowed
        or not commit(root, str(finding.get("spec_commit", ""))).ok
    )
    if mismatch:
        return RuntimeResult(None, failure("finding_binding_invalid", "finding profile or specification does not match the active review").error)
    return ok(finding)
